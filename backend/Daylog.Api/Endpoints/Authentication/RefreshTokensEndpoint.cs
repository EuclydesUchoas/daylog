...
public sealed class RefreshTokensEndpoint : IEndpoint
{
    public void MapRoute(IEndpointRouteBuilder routeBuilder)
    {
        routeBuilder
            .MapPost("v1/auth/refresh-tokens", HandleAsync)
            .WithSummary(nameof(AppMessages.Endpoint_RefreshTokensSummary))
            .WithDescription(nameof(AppMessages.Endpoint_RefreshTokensDescription))
            .AllowAnonymous()
    ...
    public static async Task<Results<Ok<Result<TokensResponseDto>>, UnauthorizedHttpResult, BadRequest<Result>, JsonHttpResult<Result>>> HandleAsync(
        [FromBody] RefreshTokensRequestDto refreshTokensRequestDto,
        [FromServices] IRefreshTokensService refreshTokenService,
        CancellationToken cancellationToken
        )
    {
        var result = await refreshTokenService.HandleAsync(refreshTokensRequestDto, cancellationToken);
